...
# modified by Percy
def get_round_keys(mk):
    return _round_keys(mk)

def reduction_mk(round_keys):
    return _mk_list_from_round_keys(round_keys)

def _mk_list_from_round_keys(round_keys):
    """
    从轮密钥中还原主密钥
    """
    rk0 = round_keys[0]
    ...
    rk3 = round_keys[3]
    _re_keys = [0,0,0,0]
    _re_keys[3]= rk3 ^ _rep_t_s(rk0 ^ rk1 ^ rk2 ^ CK[3])
    _re_keys[2]= rk2 ^ _rep_t_s(_re_keys[3] ^ rk0 ^ rk1 ^ CK[2])
    _re_keys[1]= rk1 ^ _rep_t_s(_re_keys[2] ^ _re_keys[3] ^ rk0 ^ CK[1])
    _re_keys[0]= rk0 ^ _rep_t_s(_re_keys[1] ^ _re_keys[2] ^ _re_keys[3] ^ CK[0])
    _re_mk_list = [_re_keys[0] ^ FK[0], _re_keys[1] ^ FK[1], _re_keys[2] ^ FK[2], _re_keys[3] ^ FK[3]]
    base = 0x100000000
    _re_mk = _re_mk_list[0]
    for i in _range(3):
        _re_mk = _re_mk*base+_re_mk_list[i+1]
    return _re_mk
